...
export class Theme4PageComponent implements OnInit {
  ...
  loadExercises() {
    this.userService.getExercisesByCategory('валрисьме').subscribe(data => {
  ...
  checkAnswersAndUpdateState() {
    ...
    this.userService.checkAnswers('валрисьме', userAnswers).subscribe((response: any) => { // Укажите тип ответа
