...
@Component({
  ...
  styleUrls: ['./theme1-page.component.scss']
})
export class Theme1PageComponent implements OnInit {
  ...
  loadExercises() {
    this.userService.getExercisesByCategory('валъюр').subscribe(data => {
  ...
  checkAnswersAndUpdateState() {
    ...
    this.userService.checkAnswers('валъюр', userAnswers).subscribe((response: any) => { // Укажите тип ответа
